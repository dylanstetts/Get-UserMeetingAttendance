...
param (
    [string]$UserPrincipalName = "testing@contoso.com",
...
function Export-FailedMeetings {
    ...
    $FailedMeetings | ForEach-Object {
        [PSCustomObject]@{
            ...
            Type          = $_.Type
            JoinUrl       = if ($_.OnlineMeeting) { $_.OnlineMeeting.JoinUrl } else { "" }
            OnlineMeeting = if ($_.OnlineMeeting) { $_.OnlineMeeting.ToString() } else { "" }
        }
...
function Main {
    ...
    foreach ($meeting in $meetings) {
        ...
        $joinUrl = if ($meeting.OnlineMeeting -and $meeting.OnlineMeeting.JoinUrl) { $meeting.OnlineMeeting.JoinUrl } else { $null }
        if (-not $joinUrl -or $meeting.Type -eq "seriesMaster") {
        ...
        try {
            $onlineMeeting = Get-OnlineMeetingByJoinUrl -UserId $userObjectId -JoinUrl $joinUrl
